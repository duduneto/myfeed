...
export default function Cp04({ history }) {
  // #region [setLogic] & Reducers
  // ------------------------------

  const { rdAuthUser, rdContent } = useReducer();
  const screenContent = rdContent[10];
  ...
  const GALLERY = () => {
    ...
    // set Hooks
    const callUpdateBg = useFbUpdateData();
